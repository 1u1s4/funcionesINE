...
cambiarGraficas <- function(tamFuente) {
  ...
  pkg.env$temaINE <- ggplot2::theme_gray(base_size   = pkg.env$fontSize, 
                                         base_family = "Open Sans Condensed Light") 
  + ggplot2::theme(
     text = ggplot2::element_text(family = "Open Sans Condensed Light", 
                                  face   = "plain", 
                                  colour = 'black', 
                                  size   = pkg.env$fontSize),
     axis.text.x = ggplot2::element_text(family = "Open Sans Condensed Light", 
                                         ...
                                         angle  = 0, 
                                         lineheight = 0.9),
     axis.text.y = ggplot2::element_text(family = "Open Sans Condensed Light", 
                                         colour = "black", 
                                         face   = "plain", 
                                         size   = pkg.env$fontSize, 
                                         hjust  = 0.5, 
  ...
  pkg.env$temaBarras <- pkg.env$temaBarras + ggplot2::theme(
    ...
    plot.title  = ggplot2::element_text(family = "Open Sans Condensed Light", 
                                        colour = "black", 
                                        face = "plain", 
                                        size = pkg.env$fontSize,
                                        hjust = 0.5, 
  ...
  pkg.env$temaFacets <- pkg.env$temaFacets + ggplot2::theme(
    ...
    #strip.text = ggplot2::element_text(family = "Open Sans Condensed Light", 
    #colour = "black", face = "plain", size = pkg.env$fontSize, hjust = 0.5, 
    #vjust = 1.7, angle = 0, lineheight = 0.9),
  ...
  pkg.env$temaAnillo <- pkg.env$temaAnillo + ggplot2::theme(
    plot.margin   = grid::unit(c(0,inc2mm(3.19 / 4), 0, -20), "mm"), 
    axis.line.y   = ggplot2::element_line(colour = NA),
