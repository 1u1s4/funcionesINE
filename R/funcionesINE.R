...
etiquetasBarras <- function(graph, margenIz = 0, precision = 1, cambiarNegativas = F) {
  ...
  graph <- graph + ggplot2::geom_text(data = data, ggplot2::aes(label=etiqueta, hjust=ifelse(y>=0, -0.2, 1.2), family=pkg.env$fuente),size=pkg.env$sizeText, vjust = 0.5)
...
etiquetasHorizontales <- function(graph, precision = 1, cambiarNegativas = F) {
  ...
  for (i in 1:length(posiciones)) {
    ...
    if (posiciones[[i]] == 1) {
      ...
      graph <- graph + ggplot2::geom_text(data = d, ggplot2::aes(label=ifelse(stringr::str_trim(etiqueta) == 'NA', "", etiqueta), family=pkg.env$fuente), size=pkg.env$sizeText, hjust = 0.5, vjust = 1.0 )
    }
  }
...
convertirFechasTodos <- function (lista) {
  nombres <- names(lista)
  contador <- 1
  lis <- list()
  for (x in lista){
